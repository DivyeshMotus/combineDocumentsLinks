...
def enter_links():
    links = []
    for i in range(7):
        input_link = input(f"Enter Link {i+1}: ")
        if input_link != "":
            links.append(str(input_link))
        else:
            links.append("-")
    return links

def create_folder():
    folder_path = f"downloads"
    os.makedirs(folder_path, exist_ok=True)
    os.chmod(folder_path, 0o777)
    return folder_path

def download_google_drive_file(link, output_path):
    file_counter = 1
    for link in links:
        if link == "-":
            cotinue
...
def merge_pdfs_in_folder(folder_path):
    ...
    merged_pdf_path = os.path.join(folder_path, f'merged_document.pdf')
...
def main():
    ...
    file_paths = download_files(links, folder_path)
    merged_pdf_path = merge_pdfs_in_folder(folder_path)
    shareable_link = upload_file_to_drive(drive_service, merged_pdf_path, "combinedDoc.pdf", )
